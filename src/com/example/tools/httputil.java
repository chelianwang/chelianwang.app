...
public class httputil {
	...
	public static JSONObject send(String destination,double latitude,double longtitude,List<NameValuePair> params) throws ClientProtocolException, IOException, JSONException{

		if (params==null) {//表示发送get方式请求，否则就发送Post发送请求(因为get方式不需要params)
			IP=Head+APPKEY+"&output="+outputForm+"&query="+destination+"&radius=2000&location="+latitude+","+longtitude+"&page_size="+pageSize+"&page_num=0&scope=1&mcode="+mcode;
	...
	public static void setEntityString(String entityString) {
		httputil.entityString = entityString;
	}
}
